        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
           <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
              <form @submit.prevent="handleCreatePatient">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">
                    '{{ workspaceName }}' Alanına Yeni Hasta Ekle
                  </DialogTitle>

                  <div class="mt-4 space-y-4">
                    <div>
                      <label for="patientId" class="form-label">Hasta Adı*</label>
                      <input type="text" v-model="patientData.patientId" id="patientId" class="form-input" :class="{'border-red-500': errors.patientId}" placeholder="Örn: Ahmet Yılmaz">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div>
                          ...
                          <select id="gender-select" v-model="patientData.gender" class="form-input">
                    ...
                    <div>
                      ...
                      <input type="text" v-model="patientData.race" class="form-input" placeholder="Örn: Türk">
                    </div>
                  </div>

                </div>
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button type="submit" class="btn btn-primary w-full sm:ml-3 sm:w-auto">Hastayı Oluştur</button>
...
const resetForm = () => {
  Object.keys(patientData).forEach(key => patientData[key] = '');
  errors.value = {};
};
...
const handleCreatePatient = () => {
  ...
  if (!patientData.patientId.trim()) {
    errors.value.patientId = 'Hasta Kodu zorunludur.';
    return;
  }

  // workspaceName'i de veriye ekleyerek emit et
  emit('patientCreated', {
    ...patientData,
